...
function resetComboOutputs(message = 'Import slips to get started.') {
  if (comboTimer) {
  ...
  comboMessageEl.textContent = message;
}
...
function updateComboSelectors({ suppressRecalc = false } = {}){
  ...
  if (comboClearBtn) {
    comboClearBtn.disabled = events.length === 0 || !hasParsedData;
  }
}
...
function renderAggregates({ rebuild = false } = {}) {
  ...
  if (source.length) {
    ...
    const emptyMessage = hasParsedData
      ? (aggRows.length ? 'No events match your search.' : 'No unsettled accumulator events found.')
      : 'Import slips to get started.';
    tableBody.innerHTML = `
      <tr class="empty-row">
  ...
  if (aggRows.length) {
    ...
    if (hasParsedData) {
      pillEvents.textContent = '0 events';
      pillExposure.textContent = `Total ${fmtPKR(0)}`;
    } else {
...
async function parseAndRender(htmlText){
  ...
  try{
    ...
    resetComboOutputs('Select at least two events to calculate overlap.');
    ...
    topPairs = [];
    renderAggregates({ rebuild: true });
    updateComboSelectors({ suppressRecalc: true });
    renderTopPairs();
    resetComboOutputs('Parsing failed. Check diagnostics and try again.');
  } finally {
...
resetBtn.addEventListener('click', ()=>{
  ...
  hasParsedData = false;
  aggSortState = { ...aggDefaultSort };
  resetComboOutputs('Import slips to get started.');
  renderAggregates({ rebuild: true });
  updateComboSelectors({ suppressRecalc: true });
  renderTopPairs();
});
